...
double randd(double min_value, double max_value) {
...
std::vector<double> point_on_cone(std::vector<double> penultimate_monomer_position, std::vector<double> ultimate_monomer_position) {
    ...
    return { r * cos(2 * pi * randd(0,1)),r * sin(2 * pi * randd(0,1)),r12 / 2 };
...
std::vector<double> final_monomer_position(std::vector<double> connecting_vector, std::vector<double> penultimate_monomer_position) {
    std::vector<double> z{ connecting_vector };
    std::vector<double> penultimate_monomer{ penultimate_monomer_position };
    std::vector<double> last_monomer{ vector_addition(penultimate_monomer_position,z) };

    double mod_z{ vector_modulus(z) };
    double r{ sqrt(pow(a,2) - pow(mod_z / 2,2)) };

    //std::cout << r << std::endl;
    std::cout << "position of last monomer " << std::endl;
    for (auto i : last_monomer) {
        std::cout << i << " ";
    }
    std::cout << std::endl;

    std::cout << "position of penultimate monomer " << std::endl;
    for (auto i : penultimate_monomer) {
        std::cout << i << " ";
    }
    std::cout << std::endl;
    ...
    double t{ 2 * pi * randd(0,1) };
    std::vector<double> random_position_on_cone{ r * cos(t), r * sin(t),mod_z / 2 };
    std::cout << "initial random position on cone: " << std::endl;
    for (auto i : random_position_on_cone) {
        std::cout << i << " ";
    }
    std::cout << std::endl;

    //TEST
    std::vector<double> test{ point_translation(random_position_on_cone,penultimate_monomer_position) };
    double test_distance{ vector_modulus(vector_subtraction(test,penultimate_monomer_position)) };
    //for (auto i : test) {
    //	std::cout << i << " ";
    //}
    std::cout << "conserved distance " << test_distance << std::endl;

    double alpha{ angle_with_xy_plane(z) };
    std::cout << "theta " << alpha << std::endl;
    z = clockwise_rotation_about_z_axis(z, alpha);

    double beta{ angle_with_yz_plane(z) };
    std::cout << "beta " << beta << std::endl;
    z = clockwise_rotation_about_x_axis(z, beta);

    std::cout << "transforming random point: " << std::endl;
    random_position_on_cone = clockwise_rotation_about_z_axis(random_position_on_cone, alpha);
    random_position_on_cone = clockwise_rotation_about_x_axis(random_position_on_cone, beta);


    std::vector<double> test2{ point_translation(random_position_on_cone,penultimate_monomer_position) };
    double test_distance_1{ vector_modulus(vector_subtraction(test2,penultimate_monomer_position)) };
    double test_distance_2{ vector_modulus(vector_subtraction(test2,last_monomer)) };
    std::cout << "conserved distance with last monomer after transf. " << test_distance_1 << std::endl;
    std::cout << "conserved distance with penultiamte monomer after transf. " << test_distance_2 << std::endl;

    random_position_on_cone = point_translation(random_position_on_cone, penultimate_monomer_position);
    return random_position_on_cone;
